sap.ui.define([
	...
	let that;
	return Controller.extend("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.controller.VerificadoDiferido", {
		onInit: function () {
			that = this;
			...
			oRouter.getRoute("VerificadoDiferido").attachMatched(this._onRouteMatched, this);
  ...
  var appModulePath = jQuery.sap.getModulePath(appid);
		...
		generarPedidoPorCPI: function(){
			...
			if(!pedido.Destinatario.Valor){
				...
				dfdListaMaterialesFormateada.then(function(listaMaterialesFormateada){
					let serviceParameters = {
						"HeaderSet": {
							"Header": {
								"Nav_Header_Pedido": {
			...
			Utils.loadCurrentUserData().then(function(userData){
				for(let material of listaMateriales){
					...
					listaMaterialesFormateada.push({
						...
						"TipoMensaje": "",
						"Mensaje": "",
						"Fecha_ent": pedido.FechaEntrega.Valor,
