sap.ui.define([
	...
	var oView, oSAPuser, t, Button, Dialog, oSelectedItem, data, arrjson, Respuesta, codsucursal = false,
		cantorg = [],
		selectaut;
	var mnext = [],
		msext = [];
	return Controller.extend("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.controller.CrearPedidoInterno", {
		onInit: function () {
			...
			oRouter.getRoute("interno").attachMatched(this._onRouteMatched, this);
			t = this;
			oView = this.getView();
  ...
  var appModulePath = jQuery.sap.getModulePath(appid);
			...
			$.ajax({
				...
				success: function (dataR, textStatus, jqXHR) {
					...
					for (var i = 0; i < dataR.groups.length; i++) {
						console.log(dataR.groups[i].value);
						if (dataR.groups[i].value === "AR_DP_ADMINISTRADORDEALER" || dataR.groups[i].value === "AR_DP_USUARIODEALER") {
							...
							console.log("nissan");
							oView.byId("cmbcliente").setSelectedKey("0000239911");
					...
					if (codsucursal === true) {
						...
						arr.push(obj);
						t.popError2(arr, "Error de Comunicacion");
					} else {
						t.Destinatario();
					}
		...
		VerificarUsuario: function (codsucursal) {
			var a = codsucursal;
			var arr = [];
			if (a === true) {
				var obj = {
		...
		validacampos: function () {
			var arr = [];
			if (oView.byId("cmbcliente").getValue() === "" || oView.byId("PedidoDealer").getValue() === "" || oView.byId("interlocutor").getValue() ===
				"") {
				var obj = {
					codigo: "01",
		...
		vor_: function () {
			...
			if (oView.byId("PedidoDealer").getValue().substring(0, 4) !== "VOR_") {
				oView.byId("PedidoDealer").setValue("VOR_" + oView.byId("PedidoDealer").getValue())
			}
		...
		popError2: function (obj, titulo) {
			var log = new sap.ui.model.json.JSONModel(obj);
			oView.setModel(log, "error");
			var oDialog = oView.byId("dialogError22");
			// create dialog lazily
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.view.Error2", this);
				oView.addDependent(oDialog);
			}
			oView.byId("dialogError22").addStyleClass(this.getOwnerComponent().getContentDensityClass());
			oDialog.open();
			oView.byId("dialogError22").setTitle("Error: " + titulo);
			oView.byId("dialogError22").setState("Error");
		},
		cerrarPopError2: function () {

			sap.ui.controller("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.controller.menuprincipal").bloqueo(codsucursal);
		...
		jsoncreacion: function (a, b, c, d, e, f, g) {
			...
			oView.setModel(dataT, "listadoMateriales");
			console.log(codsucursal);
			this.VerificarUsuario(codsucursal);
		},
		...
		ConsultaOdata4: function () {
            ...
            var appid = this.getOwnerComponent().getManifestEntry("/sap.app/id").replaceAll(".","/");
  			var appModulePath = jQuery.sap.getModulePath(appid);
			var region =appModulePath + '/destinations/AR_DP_REP_DEST_HANA/ODATA_masterPedido.xsodata/destinatario?$filter=SOLICITANTE%20eq%20';
			var url = region + key;
			console.log(region);
			//Consulta
			$.ajax({
		...
		ConsultaMaterial2: function () {
            ...
            var appid = this.getOwnerComponent().getManifestEntry("/sap.app/id").replaceAll(".","/");
  			var appModulePath = jQuery.sap.getModulePath(appid);
			var material = appModulePath + '/destinations/AR_DP_REP_DEST_HANA/ODATA_masterPedido.xsodata/material?$top=600&$filter=startswith(MATERIAL,%27' + key +
				'%27)';
		...
		handleValueHelp: function (oEvent) {
			...
			if (!this._valueHelpDialog) {
				this._valueHelpDialog = sap.ui.xmlfragment("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.view.DialogIn",
					this
				);
		...
		isExist: function (mat, cant) {
            var appid = this.getOwnerComponent().getManifestEntry("/sap.app/id").replaceAll(".","/");
  			var appModulePath = jQuery.sap.getModulePath(appid);
			var material = appModulePath + '/destinations/AR_DP_REP_DEST_HANA/ODATA_masterPedido.xsodata/material?$top=600&$filter=startswith(MATERIAL,%27' + mat +
				'%27)',
				cantT = cant;
			//Consulta
			$.ajax({
		...
		ConsultaStock: function () {
			...
			//antes de virificar disponibilidad y existencia
			for (var i = 0; i < json2.length; i++) {
				for (var j = 0; j < resultT.length; j++) {
			...
			console.log("crearPedidoInterno appModulePath:", appModulePath);
			$.ajax({
				type: 'POST',
				url: url,
				contentType: 'application/json; charset=utf-8',
		...
		Verificado: function () {
			...
			var b = oView.byId("PedidoDealer").getValue();
			var c = oView.byId("interlocutor").getSelectedKey();
			var d = oView.byId("VIN").getValue();
			var e = oView.byId("OR").getValue();
			var f = oView.getModel("listadoMateriales").oData;
			var g = codsucursal;
			var h = cantorg;

			sap.ui.controller("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.controller.VerificaInterno").jsoncreacion(ID_SOLICITANTE, a, b, c, d, e, f, g,
				h);

			var oRouter = this.getOwnerComponent().getRouter();
			oRouter.navTo("VerificaInterno", {
		...
		limpieza: function () {
			...
			cantorg = [];
			mnext = [];
			this.getView().byId("PedidoDealer").setValue();
			this.getView().byId("interlocutor").setSelectedKey();
			this.getView().byId("VIN").setValue();
			this.getView().byId("OR").setValue();
