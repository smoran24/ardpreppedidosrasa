sap.ui.define([
	...
	var superarr = [];
	var oMessagePopover;
	return Controller.extend("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.controller.VerificaInterno", {

		onInit: function () {
			var oRouter =
				sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("VerificaInterno").attachMatched(this._onRouteMatched, this);
		...
		jsoncreacion: function (xa, a, b, c, d, e, f, g, h) {
			oUsuariosap = xa;
			cliente = a;
			pedido = b;
			destino = c;
		...
		handleValueHelp: function (oEvent) {
			...
			if (!this._valueHelpDialog) {
				this._valueHelpDialog = sap.ui.xmlfragment("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.view.PopUpdescartadosInterno",
					this
				);
  ...
  var appModulePath = jQuery.sap.getModulePath(appid);
		...
		integrar: function () {
			...
			if (oView.getModel("validos") === undefined) {
				...
				// console.log("json2:");
				// console.log(json2); //Solo Materiales que existen
				// console.log("generar:");
				// console.log(generar); //todos los materiales con stock
				...
				// console.log("unicosExT");
				// console.log(unicosExT);
				unicosTotTem = t.getUnicos(generar);
				// console.log("unicosTotTem");
				// console.log(unicosTotTem); //unicos totales
				// console.log("unicosNeExTemp");
				// console.log(unicosNeExTemp); // unicos que no existen 
  ...
  var appModulePath = jQuery.sap.getModulePath(appid);
				...
				$.ajax({
					...
					success: function (dataR, textStatus, jqXHR) {
						...
						if (dataR.HeaderSet !== "" || dataR.HeaderSet !== undefined || dataR.HeaderSet !== null) {
							...
							console.log(dataR.HeaderSet.Header.Nav_Header_Pedido.Pedido.Mensaje);
							//	console.log(dataR.HeaderSet.Header.Nav_Header_Pedido.Pedido[0].Mensaje);
							console.log(mensaje);
							try {
		...
		popSucces: function (obj, titulo) {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.view.Succes", this);
				oView.addDependent(oDialog);
			}
		...
		atras: function () {
			...
			sap.ui.controller("AR_DP_REP_PEDIDO_RASA.AR_DP_REP_PEDIDO_RASA.controller.CrearPedidoInterno").jsoncreacion(cliente, pedido, destino,
				vin, or, arreglo, oUsuariosap);
